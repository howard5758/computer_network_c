...
void DoPeerStuff(){
	...
	while(1){
		...
		while(success){
			...
			for (file in table){
				if(file not exist locally){//download full file
					//create a download_t
					download_t task  = ;
				...
				}else if (file exist but too old){//download full file
					//same as above
				}else if (file exist and can generate by delta){//download delta file
					//create a download_t
					download_t task  = ;
					//set the name, timeStamp, file_type; intialize newPeerIp
					if(newPeerIp num >0)
						add_to_download_task();
				}else if (file being opened){
					//lock the file
				}else if (file being deleted){
					//delete the file 

					//delete the file from download task
				}
			}
...
int register_on_tracker() {
    ...
    ptp_peer_t regis_pack;  regis_pack.type = REGISTER; regis_pack.peer_ip=myIp; regis_pack.port = P2PListenPort;
